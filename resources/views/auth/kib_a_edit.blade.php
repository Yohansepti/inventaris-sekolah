...
@section('title', 'Edit Barang KIB A')

@section('content')

<h1 class="page-title">Edit Barang KIB A</h1>

<div class="input-wrapper-kib">
<form action="{{ route('kib.a.update', $kibA->kode_barang) }}" 
      method="POST" 
      class="form-grid">
    ...
    <select name="kode_sumber_dana" required>
        ...
        @foreach($sumberDana as $sd)
            <option value="{{ $sd->kode_sumber_dana }}" 
                {{ (old('kode_sumber_dana', $kibA->barang->kode_sumber_dana) == $sd->kode_sumber_dana) ? 'selected' : '' }}>
                {{ $sd->nama_sumber_dana }}
            </option>
        @endforeach
    </select>
